...
| Four-tier capability discovery and enforcement | Implemented | `Agent.run()` performs one lookup per run; agent-scoped partial overrides merge without contaminating the shared provider/model cache. The resolved matrix strips unsupported tool definitions and image markdown, bounds output tokens, excludes HITL tools without reasoning support, forwards prompt-caching support, and selects native versus prompt-guided schema handling. Unexpected disabled tool calls are sanitized, diagnosed, denied, and retried as text. Workers inherit capability bounds but exclude `delegate` from their scoped tool list. |
