...
- [`createRouteLimiters`](../../packages/server/src/http/rate-limit.ts) supplies shared process-local fixed-window route classes to the agent-configuration and inbox routers. Configuration and filesystem reads/writes plus explorer process launches have distinct limits, stable `429` responses, Express-derived network keys, and fixed-capacity stores. This is bounded single-host availability control, not authentication or a distributed quota.
...
| Durable worker-result delivery recovery | Implemented with recovery gaps | SQLite mailbox events, conditional wake, and the wake-run guard are wired; running workers and runtimes are not restartable. |
...
| Administrative and filesystem route rate limiting | Implemented, process-local | Shared configuration-read/write, filesystem-read/write, and process-launch policies protect agent and inbox routes. Per-class stores have fixed client capacity and return a stable `rate_limited` envelope; counters reset on restart and are neither authentication nor cross-process quotas. |
| Foundation security and coverage gates | Implemented in workflows; branch protection is external | Pull-request CI names Linux, Windows, full-stack, and CodeQL checks. Current-run CodeQL SARIF fails on unsuppressed security severity 7.0+, scheduled/manual ZAP fails on `FAIL` findings, and coverage enforces fixed overall/path floors plus whole-file coverage for new production sources. A maintainer must require the documented check names on `development`; ZAP is not currently a pull-request check. |
...
- route-limit allowance, exhaustion, expiry, independent network identities, bounded client state, stable pre-mutation `429` behavior, and malformed/oversized request envelopes on protected routes;
- linear TTS bracket-tag filtering under long valid and unmatched inputs, handle-based bounded reads, edit-path identity checks around handle writes, and staged-secret scanning from Git index bytes;
- dependency-audit package/advisory exceptions and their expiry behavior;
- CodeQL SARIF severity/suppression policy, CI/ZAP workflow configuration, and overall/path/new-file coverage-policy failure cases;
...
`corepack pnpm run test:coverage` uses the V8 provider across the same projects and writes text, HTML, LCOV, and JSON-summary output. Vitest retains conservative global thresholds of 24/18/19/26, while `scripts/check-coverage-policy.mjs` adds fixed overall ratchets recorded in `coverage-policy.json`, line floors for security-sensitive package paths, and a 70% whole-file line floor for newly added production sources discovered from `COVERAGE_BASE_SHA`. This is a reviewable approximation: it does not measure changed executable lines in modified files or automatically raise the recorded floors. Current measurements belong in CI or the pull request that changes them rather than in this durable architecture map.
...
Privileged operations are default-off or application-bounded: shell and network tools require explicit environment opt-in; enabled file/shell/network operations have symlink-aware authorization, handle-based bounded reads and edits on the remediated paths, byte/time/entry limits, credential-minimized subprocesses, time-bounded regex evaluation, and validated-address connection pinning with redirect revalidation. TTS tag filtering is a linear single-pass parser, and staged secret scans consume Git index blobs. The server is loopback-only by default, uses an origin allowlist and stable error envelopes, applies bounded process-local route limits, and enforces deterministic execution/resource budgets. Pull-request workflows expose named repository, full-stack, and CodeQL severity gates; scheduled/manual ZAP fails on `FAIL` results. Those names become merge gates only after a maintainer configures `development` branch protection as listed in [`docs/SECURITY.md`](../SECURITY.md). `corepack pnpm run security:audit` rejects high or critical production findings unless an explicit exception identifies the affected package and advisory with a reason and future expiry. `corepack pnpm run knip` enforces zero dead exports, orphaned files, or unlisted dependencies. Performance reporting remains informational rather than a portable timing gate. These controls do not claim authentication or process isolation; residual risks are documented in the security boundary.
