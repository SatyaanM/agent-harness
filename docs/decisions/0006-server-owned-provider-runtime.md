...
Settings changes and awaited server shutdown intentionally cancel in-flight work and require the next delivery or initialization to create a fresh runtime. Rate counters and circuits are process-local and reset on restart or accepted reconfiguration. Fixed-window admission and all-target capability intersection are conservative and may underutilize a provider, but configured limits and fallback payload safety are explicit and bounded. Capability probing consumes the same configured budgets as execution.
