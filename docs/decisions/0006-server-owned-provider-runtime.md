...
A provider attempt reserves one request and a conservative input-plus-maximum-output token estimate before network I/O. The input estimate includes complete serialized tool definitions and parameter schemas because providers count those schemas as prompt input. Exceeding either configured minute budget rejects that attempt locally as a transient rate limit, allowing another eligible provider but never creating an unbounded queue. Actual usage cannot safely undo a prior admission reservation.

After a valid settings snapshot is atomically persisted, the server aborts active parent and worker controllers, waits for their terminal cleanup, unloads all loaded sessions, discards provider health/rate state, and lazily constructs a new generation. Cancelling a worker only signals its controller; the worker remains tracked until the delegation terminal path calls `onWorkerSettled`, so reconfiguration cannot overlap durable task/mailbox cleanup. Durable transcripts, mailbox events, and task terminal handling remain authoritative; no loaded runtime silently retains stale clients.
