...
## Pull Requests & Shell Safety

- **Target Repository & Base**: When creating Pull Requests via CLI, always explicitly target `SatyaanM/agent-harness` with `--repo SatyaanM/agent-harness` and base branch `development` (`--base development`) unless explicitly requested otherwise.
