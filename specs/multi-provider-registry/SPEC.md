...
- Share circuit state across loaded sessions. Before each attempt, atomically reserve the configured request and conservative token estimate, including tool parameter schemas projected from Zod through the same provider-facing AI SDK JSON Schema conversion used for the wire payload; a denied reservation behaves as a local 429 and may fall back without waiting.
