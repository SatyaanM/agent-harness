...
**Capability discovery status:** `Agent.run()` resolves the four-tier registry once per run, or consumes one pre-resolved matrix from its runtime owner. Provider/model correlation uses the same registry target as execution, preserving opaque model IDs containing `/`; a configured provider may be probed through its declared OpenAI or Anthropic protocol without exposing its environment-sourced credential. Agent-frontmatter overrides remain scoped to that agent rather than entering the shared model cache. Only the eligible per-run tool map is advertised or executable, image markdown and HITL tools are bounded by model support, and failed configured probes fall back conservatively. Anthropic prompt caching uses AI SDK 7 provider options and an explicit system-message cache breakpoint.
