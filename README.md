...
**Capability discovery status:** `Agent.run()` resolves the four-tier registry once per run. Agent-frontmatter overrides remain scoped to that agent rather than entering the shared model cache. Unsupported tools and image markdown are omitted from provider context, model output limits bound configured output limits, and lookup failures preserve compatibility through a logged permissive fallback. Provider protocol routing remains a separate concern.
