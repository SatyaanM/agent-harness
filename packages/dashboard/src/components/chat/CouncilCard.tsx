...
export function CouncilCard({ event }: CouncilCardProps) {
  if (event.type === "council_created") {
    return (
      <div className="my-2 rounded-lg border border-purple-200 bg-purple-50 p-3 dark:border-purple-800/60 dark:bg-purple-950/40">
        <div className="flex items-center gap-2 text-sm font-medium text-purple-900 dark:text-purple-200">
          <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <title>Council created</title>
            <path
              strokeLinecap="round"
        ...
        <div className="mt-2 text-xs text-purple-700 dark:text-purple-300">
          <div>
            <span className="font-medium">Room:</span> {event.roomId}
          </div>
          <div>
            <span className="font-medium">Purpose:</span> {event.purpose}
          </div>
          <div>
            <span className="font-medium">Members:</span> {event.members.join(", ")}
          </div>
        </div>
      </div>
    );
  ...
  if (event.type === "council_message") {
    return (
      <div className="my-2 rounded-lg border border-purple-200 bg-purple-50 p-3 dark:border-purple-800/60 dark:bg-purple-950/40">
        ...
        <div className="mt-2 text-xs text-purple-700 dark:text-purple-300">
          <div>
            <span className="font-medium">From:</span> {event.taskId}
          </div>
  ...
  return (
    <div className="my-2 rounded-lg border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-700 dark:bg-zinc-900">
      <div className="flex items-center gap-2 text-sm font-medium text-zinc-900 dark:text-zinc-100">
        <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <title>Council dissolved</title>
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={2}
